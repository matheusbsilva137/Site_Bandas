<html>
	<head>
		<title>Ranking de Artistas</title>
		<meta charset="utf-8">
	...
	<div class='conteudo'>
		<p class='titulo'>Ranking de Artistas</p>
		<table>
				<tr>
					<th>Posição</th>
				...
				if(isset($_REQUEST["like"]) && $_REQUEST["like"]==true){
					...
					$rs2->execute();
					$sql = "SELECT * FROM artistas ORDER BY num_likes DESC";
					...
					$rs = $connection->prepare("SELECT * FROM artistas ORDER BY num_likes DESC");
					$rs2 = $connection->prepare("SELECT nome_banda FROM bandas where cod_banda=?");
					$pos=1;
					if($rs->execute()){
						while ($registro = $rs->fetch(PDO::FETCH_OBJ)) {
